...
Operator ingress (ports 5432, 8000) and kube-apiserver egress are handled by
`CiliumClusterwideNetworkPolicy` resources shipped with the CNPG operator
(`catalog/kubernetes/cloudnative-pg/kustomize/`) — no configuration needed here.

The `networkPolicies` toggle only controls the per-database application access
policies generated by this chart.
