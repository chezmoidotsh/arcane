...
This directory contains all Kubernetes manifests that are automatically deployed during the **bootstrap** of a Talos
Linux cluster. These components are essential for the proper functioning of the cluster and must be available from the
initial startup.
...
Custom CoreDNS configuration that allows redirecting DNS queries for `*.ts.net` domains to Tailscale's Magic DNS. This
configuration is made for TalosOS with the service CIDR `10.96.0.0/16`.
...
Component that collects resource metrics (CPU, memory) from nodes and pods in the cluster. Required for HPA (Horizontal
Pod Autoscaler) functionality and `kubectl top` commands.
