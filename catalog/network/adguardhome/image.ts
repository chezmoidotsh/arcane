...
export interface ImageArgs extends types.ImageArgs {
    /**
     * The base image to use in order to build the AdGuardHome image.
...
/**
 * An AdGuardHome Docker image baked by buildx -- Docker's interface to the improved
 * BuildKit backend.
 */
export class AdGuardHomeImage extends LocalImage {
    constructor(name: string, args: ImageArgs, opts?: pulumi.ComponentResourceOptions) {
        // Get the base image to use for building the AdGuardHome image. If no base image is provided,
