...
export class Image<T extends types.Image> extends LocalImage {
  constructor(
    name: string,
    args: ImageArgs<T> & docker_build.ImageArgs,
...
export class Application<
  T extends types.Image,
  U extends types.Image = T,
> extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    const image = new Image(
      name,
      args?.imageOpts || { push: false, load: true },
      { parent: this },
    ) as T;
    ...
    this.container = new docker.Container(
      name,
      {
        ...
        labels: pulumi
          ...
          .apply(([labels, ref]) =>
            (labels ?? []).concat([
              { label: "org.opencontainers.image.source.ref", value: ref },
            ]),
          ),
      },
      {
