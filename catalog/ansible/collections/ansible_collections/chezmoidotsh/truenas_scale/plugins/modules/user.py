...
#
# This module is specifically designed for TrueNAS Scale systems.
# SMB users require passwords - disabled passwords with SMB are not supported.
...
options:
  ...
  sudo_commands:
    description:
      ...
      - Modern TrueNAS Scale versions support both sudo_commands and sudo_commands_nopasswd simultaneously.
    type: list
    elements: str
    default: []
...
def main():
    # Verify we're running on TrueNAS Scale
    try:
    ...
    # This module only supports TrueNAS Scale
    if tn_version['name'] != "TrueNAS" or tn_version['type'] not in {"SCALE", "COMMUNITY_EDITION"}:
        print(f'{{"failed":true, "msg": "This module only supports TrueNAS Scale. Detected: {tn_version["name"]} {tn_version["type"]}"}}')
        sys.exit(1)

    # Require minimum TrueNAS Scale version (22.02+)
    if tn_version['version'] < version.parse("22.02"):
        print(f'{{"failed":true, "msg": "This module requires TrueNAS Scale 22.02 or later. Detected: {tn_version["version"]}"}}')
        sys.exit(1)

    # Use modern sudo API for Scale 22.02+
    old_sudo_api = False
