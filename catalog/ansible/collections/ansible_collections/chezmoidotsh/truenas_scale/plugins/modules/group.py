...
def main():
    ...
    if tn_version['name'] != "TrueNAS" or tn_version['type'] not in {"SCALE", "COMMUNITY_EDITION"}:
        module.fail_json(msg=f"This module only supports TrueNAS Scale. Detected: {tn_version['name']} {tn_version['type']}")

    # Require minimum TrueNAS Scale version (22.02+)
    if tn_version['version'] < version.parse("22.02"):
        module.fail_json(msg=f"This module requires TrueNAS Scale 22.02 or later. Detected: {tn_version['version']}")
    ...
    # TrueNAS Scale versions starting with 25.04 don't accept allow_duplicate_gid parameter
    TC_25_04 = version.parse("25.04")
    if tn_version['version'] >= TC_25_04:
        non_unique = None
