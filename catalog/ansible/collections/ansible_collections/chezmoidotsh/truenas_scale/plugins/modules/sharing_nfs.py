...
def main():
    # Validate TrueNAS Scale compatibility
    tn_version = setup.validate_truenas_scale()

    # Call the appropriate function to handle this.

    # TrueNAS SCALE 22.12.2 is when middlewared switched the NFS
    # parameter from 'paths' to 'path'. Use modern API for Scale.
    TC_22_12_2 = version.parse("22.12.2")
    if tn_version['version'] >= TC_22_12_2:
        return nfs2()
    else:
        return NFS1().run()
