...
description:
  ...
  - You can set the environment variable C(ANSIBLE_FACTS_MODULES) to
    C(chezmoidotsh.truenas_scale.truenas_facts) to use only this module to gather facts,
    or to C(setup, chezmoidotsh.truenas_scale.truenas_facts) to use both the standard
  ...
  - Likewise, you can set the C(ansible_facts_modules) inventory
    ...
    C(chezmoidotsh.truenas_scale.truenas_facts), or C([setup,
    chezmoidotsh.truenas_scale.truenas_facts]).
  - |
...
- name: Manually gather information
  collections: chezmoidotsh.truenas_scale
  hosts: myhost
  tasks:
...
def main():
    ...
    # Validate TrueNAS Scale environment (skip if middleware couldn't load)
    try:
        setup.validate_truenas_scale(module)
    except Exception as e:
        result['msg'] = f'TrueNAS Scale validation failed: {e}'
        result['skipped'] = True
        module.exit_json(**result)
