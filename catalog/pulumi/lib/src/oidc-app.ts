...
/**
 * Binds a Pocket-Id client to the groups allowed to sign into it. Exported
 * (not just used internally by oidcApp()) so call sites that construct
 * their `OidcClients` directly -- bypassing oidcApp() -- can still wire the
 * group restriction.
 *
