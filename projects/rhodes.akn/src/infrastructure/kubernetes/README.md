...
eBPF-based CNI: pod networking, `CiliumNetworkPolicy` enforcement, L2 announcement for LoadBalancer IPs
(`primary.ippool.yaml`, `10.0.0.64/29` — see `docs/network/vlans.md`), and the cluster's **Gateway API** implementation
...
Not a separate operator — this is the `Gateway`, HTTP→HTTPS redirect `HTTPRoute`, and wildcard `*.chezmoi.sh`
`Certificate` that Cilium's Gateway API controller (above) reconciles. Every app's own `HTTPRoute` (e.g.
`src/apps/pocket-id/main.httproute.yaml`) attaches to the `Gateway` defined here via a cross-namespace `ReferenceGrant`.
...
Publishes DNS records to the local UDM-Pro (UniFi) controller from Kubernetes `Ingress`/`Gateway` resources, via the
[external-dns-unifi-webhook](https://github.com/kashalls/external-dns-unifi-webhook) provider.

**Why this choice**: DNS records stay declarative and in sync with what's actually deployed, without a manual UniFi
console step per service. UniFi provider, not Cloudflare — these are internal-only records; Cert-Manager (above) is the
one talking to Cloudflare, for ACME DNS-01 only.
