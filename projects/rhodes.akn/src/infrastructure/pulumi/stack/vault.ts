...
// the cluster is far enough into bring-up (or recovery) to serve Vault's own
// ingress — hence this whole file is skipped while recovery mode is on (the
// `recovery` stack config). Unlike the Cloudflare/Tailscale token files, there is
// no "external resource" half here to create early: every resource in this file
// is a Vault-side resource.
if (!config.getBoolean("recovery")) {
	...
	// `rhodes.akn-eso-role` to read the `rhodes.akn/` KV mount.
	//
	// TODO(rhodes.akn): amiya.akn's ClusterVaultComponent call bound an
	// "amiya.akn-authelia-policy" here (Authelia depended on it; Vault's
	// tokenPolicies is a full replace, so it couldn't just be dropped there).
	// rhodes.akn doesn't run Authelia — Pocket-Id is the SSO — so this is left
	// out. Revisit if/when this cluster needs an equivalent additionalPolicyNames
	// binding, via additionalPolicies (Pulumi-owned) rather than by name.
