...
| **Rationale** | "Only the ports every Talos node needs are here …"    | `partials/<section>.<subject>.hbs` | it is guarded on its subject |
| **Framing**   | "Pool membership _is_ the permission"                 | `template.hbs` / `partials/`       | it states an invariant       |

**The rule that keeps the document honest: static prose in `template.hbs`/`partials/` may state an invariant or a
rationale, but never a count, a name, or a list.** "Three identities" rots the moment a fourth appears, so it is
derived. "Pool membership is the permission" holds regardless of how many pools exist, so it can be written by hand.
...
`partials/firewall.talos.hbs` — included from the section partial **inside that resource's loop iteration**, guarded on
the subject:
...
4. **Render** — `render.ts` compiles `template.hbs` with every `partials/*.hbs`.
5. **Write** — `generate.ts` writes the Markdown to `projects/chezmoi.sh/docs/PROXMOX-VE.md`.
...
| Path                               | Purpose                                                                                           |
| ---------------------------------- | ------------------------------------------------------------------------------------------------- |
| `generate.ts`                      | Entry point (`mise run proxmox:docs:generate`). Reads state, builds the context, renders, writes. |
...
| `render.ts`                        | Compiles `template.hbs` + `partials/*.hbs`.                                                       |
...
| `template.hbs`                     | Document skeleton; includes the partials.                                                         |
| `partials/<section>.hbs`           | One section each. `partials/access.hbs` registers as `{{> access}}`.                              |
| `partials/<section>.<subject>.hbs` | Rationale for one named resource, included under its own heading.                                 |
...
**Safety net:** if `proxmox:endpoint` is unset, `generate.ts` logs and exits cleanly instead of failing —
`pulumi:apply`'s post-task chain covers the whole shared stack, so a hard failure here would block applies for
...
3. Add `partials/<section>.hbs` and reference it from `template.hbs`.
4. Wire it into `buildContext()` in `generate.ts`.
