...
2. **Extract plain data**: `extract.ts` groups the exported resource list by Pulumi type token
   ...
   `templates/root.hbs`/`templates/partials.*.hbs` expect.

3. **Render**: `render.ts` compiles `templates/root.hbs` (with every `templates/partials.*.hbs`) via Handlebars —
   byte-identical logic to `../truenas-docs/render.ts`.

4. **Write**: `index.ts` writes the rendered Markdown straight to `projects/chezmoi.sh/docs/PROXMOX_BACKUP_SERVER.md`
...
| `templates/partials.overview.hbs`        | PBS instance overview: what it's for, that it runs as a Proxmox VE VM, and the live datastore count.                                                                                           |
| `templates/partials.glossary.hbs`        | Static "Key terms" primer (datastore, chunk, prune, GC, verify, notification target/matcher) — not data-driven, read once by anyone unfamiliar with PBS before the data-driven sections below. |
| `templates/partials.datastore.hbs`       | Each datastore's S3 backend, local cache path, GC schedule, and notification delivery mode.                                                                                                    |
| `templates/partials.retention.hbs`       | Prune jobs (retention summary per job) and verify jobs (schedule, outdated-after window).                                                                                                      |
| `templates/partials.notifications.hbs`   | Notification targets (name + kind only — see "A secret this provider doesn't mark as secret" above) and their routing matchers.                                                                |
| `templates/partials.access.hbs`          | Users, API tokens (never their one-time secret), and the ACL bindings table.                                                                                                                   |
| `templates/partials.pve-integration.hbs` | Per-datastore, copy-pasteable instructions (UI and `pvesm add` CLI) for adding it as Proxmox VE storage.                                                                                       |
...
`index.ts` logs a message and exits cleanly instead of failing — `pulumi:apply`'s post-task chain covers the whole
...
1. Create a new partial in `./templates/` (e.g., `templates/partials.remotes.hbs`, for `pbs.Remote` sync targets).
2. Add a placeholder in `templates/root.hbs`: `{{> remotes}}`.
...
4. Use the helper in a partial via `{{myHelper arg1 arg2}}`. Use triple braces (`{{{myHelper ...}}}`) if the output
   ...
   precomputed `retentionSummary` string — see `templates/partials.retention.hbs`).
