...
| `partials/overview.hbs`    | TrueNAS instance overview: what it's for, that it runs as a Proxmox VM, and the live pool count/names.                                                                                                                                                                                                   |
| `partials/glossary.hbs`    | Static "Key terms" primer (pool, dataset, snapshot, scrub, compression/record size, ACL) — not data-driven, read once by anyone unfamiliar with ZFS/TrueNAS before the data-driven sections below.                                                                                                       |
...
| `partials/shares.hbs`      | NFS and SMB shares with mount options, permissions, and purpose labels. Each subsection (and the whole `## Shares` intro) is omitted when its share list is empty.                                                                                                                                       |
| `partials/permissions.hbs` | Service identities, the 5 NFS4 ACL templates this stack manages, and the dataset → template guide for manually applying them.                                                                                                                                                                            |
| `partials/backups.hbs`     | The three-layer backup strategy (scrub, snapshot, off-site sync): what each layer protects against, how it works, and its live cron schedule as a bullet list.                                                                                                                                           |
