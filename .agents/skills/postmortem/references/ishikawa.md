...
When the failure has multiple potential cause categories. Map causes under six standard
categories, then drill into the most plausible ones.
...
Each sub-cause branches off a category spine; all spines converge on the failure event
(the "fish head"). Replace the placeholder causes above with the candidates you discover
...
The structure forces breadth and prevents premature narrowing — especially useful when a team
is stuck in blame mode, because it distributes attention across the whole system rather than
fixating on one person or component.
