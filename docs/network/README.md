...
* **eth0 → vmbr1 (VLAN 5, no node IP)** — Cilium L2 LoadBalancer ARP announcements only.
  ...
  the UDM Pro.
* **eth1 → per-cluster SDN VNet (VXLAN)** — all node management traffic (kubelet, etcd,
  inter-node). Fully isolated per cluster; SNAT lets nodes reach the Proxmox API for the
  CSI volume driver.

The Proxmox SDN VXLAN zone (`10.128.0.0/16`) provides per-cluster L2 isolation without
any UDM Pro configuration changes per new cluster. See [ADR-014](../decisions/014-network-topology.md)
...
| Proxmox SDN VXLAN            | `10.128.0.0/16`                    |
