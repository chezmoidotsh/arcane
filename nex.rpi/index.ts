...
const alpn = new alpine.Image("alpine", { push: true, platforms: ["linux/amd64", "linux/arm64"] });
const yldp = new yaldap.Application("security.yaldap",
    {
        imageOpts: {
            push: true, baseImage: alpn,
            ...
            // transformation: image => utils.InjectAssets(
            //     image,
            //     {
            //         source:
            //             config.require("sh.chezmoi.environment") == "live"
            //                 ? new pulumi.asset.FileAsset("config/live")
            //                 : new pulumi.asset.RemoteAsset("https://raw.githubusercontent.com/chezmoi-sh/yaldap/main/pkg/ldap/directory/yaml/fixtures/basic.yaml"),
